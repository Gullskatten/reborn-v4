...
@export var smooth_zoom = 0.3 : set = set_smooth_zoom;
...
func _ready():
	GlobalCameraSettings.connect("forced_camera_position_changed", _on_force_update_position)
	GlobalCameraSettings.connect("forced_zoom_changed", _on_force_update_zoom)
...
func _on_force_update_position(pos):
	if(cameraPosition != null):	
		cameraPosition.set_deferred("global_position", pos)

func _on_force_update_zoom(zoom):
	set_smooth_zoom(zoom)
